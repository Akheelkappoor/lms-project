...
class User(UserMixin, db.Model):
    ...
    def has_permission(self, permission):
        ...
        # For tutor - expanded permissions  
        if self.role == 'tutor':
            tutor_permissions = [
                'class_management', 'attendance_management', 
                'profile_management', 'communication', 'schedule_management',
                'student_management',  # Can manage their own students
                'report_generation'   # Can generate their reports
            ]
            return permission in tutor_permissions
        
        # For student - basic permissions
    ...
    def create_default_admin():
        ...
        admin = User.query.filter(User.email.ilike(Config.DEFAULT_ADMIN_EMAIL)).first()
        if not admin:
            admin = User(
                username='admin',
    ...
    def get_profile_picture_url(self):
        """Get the profile picture URL with proper S3 handling"""
        if self.profile_picture:
            # If it's already a full URL, return as is
            if self.profile_picture.startswith('http'):
                return self.profile_picture
            # If it's a relative path, construct S3 URL
            elif self.profile_picture.startswith('/'):
                return self.profile_picture
            else:
                # Handle S3 bucket URLs
                return self.profile_picture
        return None
