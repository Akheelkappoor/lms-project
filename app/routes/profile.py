            ...
            if form.profile_picture.data:
                try:
                    ...
                    s3_url = upload_file_to_s3(
                        ...
                        folder=f"{current_app.config['UPLOAD_FOLDER']}/profiles"
                    )
                    if s3_url:
...
@bp.route('/profile/documents/<document_type>/view')
@login_required
def view_profile_document(document_type):
    """Allow tutors to view their own documents securely"""
    try:
        if current_user.role != 'tutor':
        ...
        if not document_url:
            flash('Document URL not found.', 'error')
            return redirect(url_for('profile.manage_documents'))
        ...
        if signed_url:
            ...
            flash('Failed to generate secure access URL.', 'error')
            return redirect(url_for('profile.manage_documents'))
            
    except Exception as e:
        current_app.logger.error(f'Error viewing profile document: {str(e)}')
