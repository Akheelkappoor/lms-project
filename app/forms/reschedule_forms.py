...
class RescheduleRequestSearchForm(FlaskForm):
    ...
    tutor = QuerySelectField(
        'Tutor',
        query_factory=lambda: sorted(
            Tutor.query.filter(Tutor.status=='active').all(),
            key=lambda t: t.user.full_name
        ),
        get_label=lambda tutor: tutor.user.full_name,
        allow_blank=True,
        blank_text='— All Tutors —'
    )
    ...
    def __init__(self, department_id=None, *args, **kwargs):
        ...
        self.tutor.choices = [('', 'All Tutors')] + [(t.id, t.user.full_name) for t in tutors]
